# Modified to get only sunkpos matching chosen HiC phase for each contig
import os
import system
import parsecsv
# import streams
...
proc main() =
  ...
  var contigphasefile = args[2]
  type contPhaseTup = tuple
    contig: string
    phase: char
  var bestContigs = initTable[string,contPhaseTup]()
  var x,y: CsvParser
  x.open(newGZFileStream(contigfile), contigfile, separator = '\t')
  while x.readRow():
    bestContigs[x.row[0]] = (x.row[1],x.row[5][0])
# x.row[5] is read hap assignment
  x.close()
  
  var
    h = newGzFileStream(contigphasefile, fmRead)
    phases = initTable[string,char]()
  open(y, h, contigphasefile, separator = '\t')
  while readrow(y):
...
# var contigs : HashSet[string]
  ...
  while x.readRow():
    if x.row[0] != rname_prev: 
      ...
      stdout.flushFile()
      (bestContig, readHap) = bestContigs.getOrDefault(x.row[0])
      contHap = phases.getOrDefault(bestContig)
  #      output outLines from 0 to i-1
      i = 0
    # outLines[i] = x.row
    # inc i
    rname_prev = x.row[0]
    if (x.row[2] == bestContig) and (readHap == contHap):
      outLines[i] = x.row
      inc i
